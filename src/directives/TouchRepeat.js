...
export default {
  ...
  bind(el, { modifiers, value, arg }) {
    ...
    if (Platform.has.touch === true) {
      const opts =
        listenOpts[
          "notPassive" + (modifiers.capture === true ? "Capture" : "")
        ];
  ...
  unbind(el, { modifiers }) {
    ...
    if (ctx !== void 0) {
      ...
      if (
        ...
        (ctx.event !== void 0 && ctx.event.repeatCount > 0)
      ) {
        document.documentElement.style.cursor = "";
        document.body.classList.remove("non-selectable");
      }
      ...
      delete el[
        el.__qtouchrepeat_old ? "__qtouchrepeat_old" : "__qtouchrepeat"
      ];
    }
  }
};
