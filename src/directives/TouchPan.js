...
export default {
  ...
  bind(el, { value, modifiers }) {
    ...
    const touchPassiveStr =
        modifiers.mightPrevent !== true && modifiers.prevent !== true
          ? 'passive'
          : 'notPassive',
      touchEvtOpts =
        listenOpts[
          touchPassiveStr + (modifiers.capture === true ? 'Capture' : '')
        ]
    ...
    if (modifiers.mouse === true) {
      el.addEventListener(
        ...
        ctx.mouseStart,
        listenOpts[
          `notPassive${modifiers.mouseCapture === true ? 'Capture' : ''}`
        ]
      )
    }
